  ...
  return Object.entries(PKG.exports).flatMap(([name, entryPoints]) => {
    ...
    if(entryPointKeys.includes("source")) {
      ...
      if(!hasAtLeastOneRequiredField) {
        throw new Error(
          `A \`source\` field is defined without a provided \`${name}\` module entry point. Make sure to define at least one entry point (including ${requiredFieldIfSourceDefined.join(
            ", ",
          )})`,
        );
      }
...
const createMainConfig = (
  entryPoints: Required<Pick<EntryPoints, "source">> & Partial<Pick<EntryPoints, "default" | "import" | "require">>,
): RollupOptions => {
  const output = [
    entryPoints.require && {
      format: "cjs",
